...
from models import Artist, Venue, Area, Show
...
def search_venues():
    ...
    data = Venue.query.filter(Venue.name.ilike('%{}%'.format(q)))
...
def show_venue(venue_id):
    ...
    # jsondata = list(filter(lambda d: d['id'] == venue_id, [data1, data2, data3]))[0]
...
def create_venue_submission():
    ...
    try:
        ...
        area = Area.query.filter_by(city=city, state=state).first() \
            if Area.query.filter_by(city=city, state=state).first() \
            else Area(city=city, state=state)
        venue = Venue(name=name, address=address, phone=phone, image_link=image_link, genres=genres,
                      facebook_link=facebook_link, website=website_link, seeking_talent=seeking_talent,
...
def search_artists():
    ...
    q = request.form.get('search_term', '')
    data = Artist.query.filter(Artist.name.ilike('%{}%'.format(q)))
...
def show_artist(artist_id):
    ...
    # json_data = list(filter(lambda d: d['id'] == artist_id, [data1, data2, data3]))[0]
...
def edit_artist(artist_id):
    ...
    artist.genres = list(artist.genres[1:-1].split(","))
...
def edit_artist_submission(artist_id):
    ...
    try:
        ...
        artist.seeking_venue = True if request.form.get("seeking_venue", False) else False
...
def edit_venue(venue_id):
    ...
    venue.genres = list(venue.genres[1:-1].split(","))
...
def edit_venue_submission(venue_id):
    ...
    try:
        ...
        venue.seeking_talent = True if request.form.get("seeking_talent", False) else False
...
def create_artist_submission():
    ...
    try:
        ...
        artist.website = request.form['website_link']
        artist.facebook_link = request.form['facebook_link']
        artist.seeking_venue = True if request.form.get("seeking_talent", False) else False
        artist.seeking_description = request.form['seeking_description']
        artist.image_link = request.form['image_link']
...
def create_show_submission():
    ...
    try:
        if Venue.query.get(request.form['venue_id']) is None:
            flash('Venue Id provided doesn\'t exist.')
            form = ShowForm(obj=request.form)
            return render_template('forms/new_show.html', form=form)
        if Artist.query.get(request.form['artist_id']) is None:
            flash('Artist Id provided doesn\'t exist.')
            form = ShowForm(obj=request.form)
            return render_template('forms/new_show.html', form=form)
        show = Show()
